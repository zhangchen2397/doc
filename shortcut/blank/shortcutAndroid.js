...
;(function(root, factory) {
    ...
    var styleStr = [
        '#shortcut-a-pannel {display:none; z-index:9999; position:fixed; border:1px solid solid rgba(215,215,215,0.9); padding:0 10px; height:100px; bottom:10px; background-color:#EBEBEB; opacity:0.9; border-radius:3px;}',
        '#shortcut-a-pannel .con { position:relative; }',
        '#shortcut-a-pannel .ic-closebtn {position:absolute; right:0; top:8px; z-index:3; width:11px; height:11px; background:transparent url(http://3gimg.qq.com/wap30/infoapp/touch/itravel/images/img/ic-closebtn.png) no-repeat scroll left top; -webkit-background-size: 11px auto;}',
    ...
    shortcut.prototype = {
        ...
        _setPos: function() {
            var me = this,
                ...
                elWidth = this.getSize(this.el).width;

            this.el.style.left = (clientWidth - elWidth) / 2 + 'px';
        },
        ...
        getSize: function(el) {
            var elStyle = el.style,
                ...
                oldDisplayStyle = getStyle(el, 'display'),
                oldPositionStyle = getStyle(el, 'position'),
                oldVisibilityStyle = getStyle(el, 'visibility');
            ...
            elStyle.display = oldDisplayStyle;
            elStyle.position = oldPositionStyle;
            elStyle.visibility = oldVisibilityStyle;
            ...
            function getStyle(el, styleName) {
                ...
                if (el.style[styleName]) {
                    styleValue = el.style[styleName];
                } else {
                    if (el.currentStyle) {
                        styleValue = el.currentStyle[styleName];
                    } else {
                        styleValue = window.getComputedStyle(el, null)[styleName];
                    }
                }
